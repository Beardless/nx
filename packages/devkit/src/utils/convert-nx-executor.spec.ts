...
// TODO(jack): skipped for now to see how e2e tests run
xdescribe('Convert Nx Executor', () => {
  // Change cwd for tests or else tests will fail when loading plugins from nx.json.
  // Loading of `@nx/playwright/plugin`, for example, will load the `plugin.d.ts` file
  // from node_modules (since we're using ts-jest),
  let oldCwd: string;
